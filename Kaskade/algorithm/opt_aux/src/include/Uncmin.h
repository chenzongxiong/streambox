 ...
 */
template<class V, class M, class FUNC> void Uncmin<V, M, FUNC>::SetNumDigits(int ndigit)
{
  this->ndigit = ndigit;
}
    ...
    V &sc)
{
  ...
  else
  {
    ...
    if (eta*rnwtln <= dlt)
    {
      // TAKE PrecondType::PARTIAL STEP IN NEWTON DIRECTION
      for (i = 1; i <= n; i++)
      {
        sc(i) = (dlt/rnwtln)*p(i);
      }
...
template<class V, class M, class FUNC> void Uncmin<V, M, FUNC>::qrupdt(M &a, V &u, V &v)
{
  ...
  // (K-1) PrecondType::JACOBI ROTATIONS TRANSFORM
  // R + U(V+) --> (R*) + (U(1)*E1)(V+)
  ...
  // (K-1) PrecondType::JACOBI ROTATIONS TRANSFORM UPPER HESSENBERG R
