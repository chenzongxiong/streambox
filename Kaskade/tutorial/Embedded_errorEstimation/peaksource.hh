...
class PeaksourceFunctional: public FunctionalBase<VariationalFunctional>
{
public:
  using Scalar = RType;
...
///
  ...
  {
  public:
    DomainCache(PeaksourceFunctional<RType,AnsatzVars> const&,
    ...
    void evaluateAt(Position const& x, Evaluators const& evaluators) 
    {
      ...
      if (dim==2) {
      	...
      	h   = exp( -p*( (xglob[0]-0.5)*(xglob[0]-0.5) + (xglob[1]-0.5)*(xglob[1]-0.5) + (xglob[2]-0.5)*(xglob[2]-0.5) ) );
      	hX  = -2*p*(xglob[0]-0.5)*h;
      	hXX = -2*p*h - 2*p*(xglob[0]-0.5)*hX;
      	hY  = -2*p*(xglob[1]-0.5)*h;
      	hYY = -2*p*h - 2*p*(xglob[1]-0.5)*hY;  
...
///
  ...
  public:
    ...
    BoundaryCache(PeaksourceFunctional<RType,AnsatzVars> const&,
                  typename AnsatzVars::VariableSet const& vars_,
                  int flags=7):
      data(vars_), e(0)
    {}
...
/// \example peaksource.cpp
/// show the usage of PeaksourceFunctional describing a stationary heat transfer problem with peak formed source, 
