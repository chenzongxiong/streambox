...
template<template<class> class BundleT>
void UnboundedInMem<Event1, BundleT>::ExecEvaluator(int nodeid, EvaluationBundleContext *c, shared_ptr<BundleBase> bundle_ptr) {
    /* instantiate an evaluator */
    UnboundedInMemEvaluator<Event1, BundleT> eval(nodeid);
    eval.evaluate(this, c, bundle_ptr);
}
...
template
void UnboundedInMem<Event1, RecordBundle>::ExecEvaluator(int nodeid, EvaluationBundleContext *c, shared_ptr<BundleBase> bundle_ptr);
//george: instantiation
template
void UnboundedInMem<Event1, RecordBitmapBundle>::ExecEvaluator(int nodeid, EvaluationBundleContext *c, shared_ptr<BundleBase> bundle_ptr);
