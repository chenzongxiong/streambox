...
struct mark_entry {
    string msg;
    ptime ts;
    mark_entry(string const &msg, ptime const &ts) : msg(msg), ts(ts) {}
};
//#endif
...
struct BundleBase {
    ...
    int side_info = 0;
    int get_side_info() { return side_info; }
    void set_side_info(int i) { side_info = i; }
    ...
    void dump_markers() {
        ...
        cout << "dump markers: <<<<<<<<<<<<<<"
             << to_simplest_string(this->min_ts) << endl;
        for (auto &e : this->marks) {
            cout << "dump markers:" << i++ << "\t" << e.msg << " ";
            ...
            if (i == 1) {
                ...
                cout << " +" << (e.ts - last).total_milliseconds() << " ms"
                     << endl;
            }
        ...
        cout << "dump markers: >>>>>>>>>>>>>>>>>>>>>>>>>>>>";
        cout << "total " << (last - first).total_milliseconds() << " ms"
             << endl;
        //#endif
    }
    //#endif
...
static const char *puncref_key_[] = {"X", "U", "C", "R", "A", ""};
...
struct bundle_container {
  ...
  public:
    ...
    int get_side_info() { return side_info.load(); }

    int has_punc = 0; // 0: this container doesn't have pun cyet
    // 1: this container already has a punc

    shared_ptr<Punc>
        punc; /* unsafe to access this from outside w/o protection */
  ...
  public:
    ...
    /* punc 	 punc_refcnt
     * ---------------------
     * nullptr 	  -2				punc invalidated by upstream (i.e. will never
     * be
     * assigned)
     * nullptr		-1				punc never assigned
     * valid			2					assigned, not emitted
     * (==
     * retrieved)
     * valid			1			  	assigned, emitted,
     * not
     * consumed
     * valid			0					consumed
     */
    ...
    /* OBSOLETED:
     * Staged bundles are output produced by processing bundles in this
     * container;
     * however, if there is an earlier container which may assign a punc to
     * downstream later, depositing these bundles to downstream at this
     * time.will
  ...
  public:
    //	bundle_container () : punc(nullptr), bundles(), punc_refcnt(-1),
    // refcnt(0) { }
    ...
    bundle_container(int side_info = 0)
        ...
        : side_info(side_info),
          punc(nullptr),
          downstream(nullptr) // fix warning
    {
        // side_info = 0;
        punc_refcnt = PUNCREF_UNDECIDED;
        refcnt = 0;
        //		bundles.reserve(80);  // for vector. actually make things
        //slightly
        // worse.
    }
...
#define ASSERT_VALID_NUMA_NODE(node)                                           \
    do {                                                                       \
  ...
  public:
    ...
    shared_ptr<BundleBase> getOneBundle(int node = -1) {
        ...
        if (bundles.empty() || node == -2) {
            /* debugging */
            EE("%s: bug? : bundles %u %u %u %u; min_ts %s", getName().c_str(),
...
/* sanity check */
        ...
        W("%s: bundle %s, after: bundles %u %u %u %u; min_ts %s",
          getName().c_str(), to_simplest_string(bundle->min_ts).c_str(),
          (unsigned int)(numa_bundles[0].size()),
          (unsigned int)(numa_bundles[1].size()),
          (unsigned int)(numa_bundles[2].size()),
          (unsigned int)(numa_bundles[3].size()),
...
class PValueBase : public PValue {
  public:
    ...
    bool isFinishedSpecifyingInternal() { return _finishedSpecifying; }

    void finishSpecifying() { _finishedSpecifying = true; }
...
class PDone : PValueBase {
  ...
  public:
    static PDone *in(Pipeline *pipeline) { return new PDone(pipeline); }

    string getName() { return "pdone"; }
...
class PCollection : public PValueBase {
  public:
    ...
    enum class IsBounded { BOUNDED, UNBOUNDED };
    ...
    // return a ref
    PCollection *setName(string name) {
        PValueBase::setName(name);
        return this;
    }
    ...
    WindowingStrategy *getWindowingStrategy() { return _windowingStrategy; }
  ...
  public:
    PCollection(Pipeline *p)
        : PValueBase(p), _isBounded(IsBounded::BOUNDED),
          _windowingStrategy(NULL) {}

    PCollection(Pipeline *p, string name)
        : PValueBase(p, name), _isBounded(IsBounded::BOUNDED),
          _windowingStrategy(NULL) {}
};
...
class PBegin : public PValueBase {
  public:
    static PBegin *in(Pipeline *p) { return new PBegin(p); }

    PValue *apply(PTransform *p) { return Pipeline::applyTransform(this, p); }

    PValue *apply(string name, PTransform *p) {
        return Pipeline::applyTransform(name, this, p);
    }
 ...
 *
 * <p>The timestamp of a value determines many properties, such as its
 * assignment to
 * windows and whether the value is late (with respect to the watermark of a
 * {@link PCollection}).
 *
...
template <class T> class WindowedValue {
  protected:
    T _value;
    PaneInfo _pane;
...
template <class T> class MinTimestampWindowedValue : public WindowedValue<T> {
  public:
    MinTimestampWindowedValue(T value, PaneInfo pane)
        : WindowedValue<T>(value, pane) {}

    ptime getTimestamp() { return BoundedWindow::TIMESTAMP_MIN_VALUE; }
};
...
class ValueInEmptyWindows : public MinTimestampWindowedValue<T> {
  public:
    ...
    // xzl: upcasting?
    WindowedValue<T> *withValue(T value) {
        return (WindowedValue<T> *)new ValueInEmptyWindows<T>(value,
                                                              this->_pane);
    }
...
template <class T> class TimestampedWindowedValue : public WindowedValue<T> {
  protected:
    ptime _ts;
...
class TimestampedValueInMultipleWindows : public TimestampedWindowedValue<T> {
  ...
  public:
    ...
    // xzl: upcasting?
    WindowedValue<T> *withValue(T value) {
        return (WindowedValue<T> *)new TimestampedValueInMultipleWindows<T>(
            value, this->_ts, _windows, this->_pane);
    }

    vector<BoundedWindow *> *getWindows() { return _windows; }
};
...
template <class T>
static WindowedValue<T> *of(T value, ptime ts, vector<BoundedWindow *> *windows,
                            PaneInfo pane) {
    if (windows->empty() && BoundedWindow::TIMESTAMP_MIN_VALUE == ts) {
        ...
        return new TimestampedValueInMultipleWindows<T>(value, ts, windows,
                                                        pane);
}
}
...
template <typename T1, typename T2> struct Element2 {
    ...
    Element2(Tuple cols, ptime ts = max_date_time, ptime ts2 = max_date_time,
             BoundedWindow *window = nullptr)
        : cols(cols), ts(ts), ts2(ts2), window(window) {}
};

template <typename T1, typename T2> struct Header2 {
...
template <typename T> struct Record {
    ...
    Record(const T &data, const ptime &ts) : ts(ts), data(data) {}
...
#endif
template <typename T> struct RecordBundle : public BundleBase {
    ...
    typename VecT::iterator begin() { return content.begin(); }

    //  typename vector<RecordT>::iterator end() {
    typename VecT::iterator end() { return content.end(); }
...
template <>
inline void RecordBundle<creek::tvpair>::add_record(const Window &win,
                                                    const RecordT &rec) {
    add_record(rec);
}
 ...
 */
template <typename T> struct RecordBitmapBundle : public RecordBundle<T> {
    using RecordT = Record<T>;
      ...
      public:
        ...
        iterator &operator++() {
            if (index < bundle->content.size()) {
                index++;
                while (index < bundle->content.size() &&
                       !bundle->selector[index])
                    index++;
            }
            return *this;
        }
    ...
    iterator end() { return iterator(this, this->content.size()); }
...
template <typename T1, typename T2> struct BundleCol2 : public BundleBase {
...
struct Window {
    ...
    Window(ptime start) : start (start-epoch), duration(seconds(1)) {}
    ...
    inline ptime window_end() const { return epoch + start + duration; }
 ...
 * see
 * https://www.threadingbuildingblocks.org/docs/help/tbb_userguide/More_on_HashCompare.html
 */
struct WindowHashCompare {
    static size_t hash(const Window &w) { return (size_t)(w.duration.ticks()); }
...
template <typename T> struct SessionWindow : public Window {
    using RecordT = Record<T>;
    ...
    // since we only store pointers (not the actual records in the input
    // bundle),
     ...
     */
    bool try_add_record(RecordT const *rec, shared_ptr<InputBundleT> bundle) {
        assert(rec);
    ...
    bool merge(SessionWindow const &other) {
        ...
        if (oend > end) { // extend window end
            //      assert(ostart > start);   // possible when we merge window
            //      sets
            this->duration += (oend - end);
        }

        this->ranges.insert(this->ranges.end(), other.ranges.begin(),
                            other.ranges.end());
...
template <typename T> struct WindowRecComp {
    // is the session window *strictly* smaller than the record's window?
...
template <typename T> struct WindowStrictComp {
 ...
 */
template <class KVPair> struct ReducedKVPairCompLess {
...
template <class KVPair> struct ReducedKVPairCompLess1 {
    bool operator()(const KVPair &lhs, const KVPair &rhs) const {
        /* since we are going to maintain a min heap */
        return lhs.second < rhs.second;
    }
...
template <typename K, typename V> struct WindowedKVOutput {
 ...
 */
template <typename T> struct WindowsBundle : public BundleBase {
    ...
    void add(shared_ptr<WindowFragment<T>> output) {
        ...
        if (output->max_ts > max_ts)
            max_ts = output->max_ts;
    }

    /* can be called with (win, val), for which a record will be auto
     * constructed
     * based on val */
    void add_record(const Window &w, const RecordT &val) { add_value(w, val); }
    ...
    void show_window_sizes() {
        ...
        for (auto &&it = vals.begin(); it != vals.end(); it++) {
            ...
            assert(output);
            EE("window %d (start %s) size: %lu", output->id,
               to_simple_string(output->w.start).c_str(), output->size());
            sum += output->size();
        }
        EE("----- total %lu", sum);
    }
    ...
    typename map<Window, shared_ptr<WindowFragment<T>>, Window>::iterator
    begin() {
        return vals.begin();
    }
...
struct WindowsKeyedBundle : public BundleBase {
    ...
    typename map<Window, WindowFragmentKV, Window>::iterator
    begin() {
        return vals.begin();
    }
    ...
    void print() {
        std::cout << "windowkeyed bundle" << std::endl;
    }
};
 ...
 */
template <typename KVPair> class window_map_iterator {
  ...
  public:
    ...
    bool operator!=(window_map_iterator const &other) const {
        ...
        return (it_win != other.it_win ||
                it_kvcontainer != other.it_kvcontainer);
    }
    ...
    window_map_iterator &operator++() {
         ...
         * 1. undefined: if we already hit the end of the last window and still
         * try
         * to ++;
         * 2. bug: we somehow stopped at the end of an intermediate window
         */
        if (it_kvcontainer == it_win->second.vals.end()) {
            EE("bug: distance to win end %lu",
               std::distance(it_win, _windows->end()));
        }
    ...
    Window const &get_current_window() const { return it_win->first; }
};
...
template <typename KVPair> class window_map_iterator2 {
...
struct __attribute__((packed)) Event1 {
    char user_id[36];   // 36
    char page_id[36];   // 36
    char ad_id[36];     // 36
    char ad_type[16];   // 16
    ...
    // total 136 bytes
    void print() {
        std::cout << user_id << " " << page_id << " " << ad_id << " " << ad_type
                  << " " << event_type << " " << ip << std::endl;
    }
};
...
using SetArray =
    std::array<tbb::concurrent_unordered_set<creek::string>, NUM_SETS>;
