...
int main(int argc, char *argv[])
{
  ...
  std::cout << "Start sst transfer tutorial program with exact damped Newton iteration " << std::endl;
  ...
  std::cout << "Start sst transfer tutorial program" << std::endl;
...
// using H1Space = FEFunctionSpace<ContinuousHierarchicMapper<Scalar,LeafView> >;
  ...
  using NonlinearSolver = NleqSolver<Grid,Functional,VariableSet,Spaces>;
...
//  writeVTKFile(leafGridView,x,"graph/sst_start",options,std::min(order,2));
  writeVTKFile(x,"graph/sst_nleqErr_start",IoOptions().setOrder(std::min(order,2)).setPrecision(7));
...
//   monitorStream.open("sst.mon",std::fstream::out);
//   if ( !monitorStream.is_open() )
//   {
//      std::cout << " Failed to open monitorStream file sst.mon" << std::endl;
//      exit(-1);
//   };
...
//  nonlinearSolver.setScalingVector(&scale);
  nonlinearSolver.setOutFilePrefix((std::string) "graph/sst_nleqErr");

  boost::timer::cpu_timer nonlinTimer;
  nonlinearSolver.nleqErr(gridManager,F,variableSet,&x,spaces);
...
//   monitorStream << " The total number of iterative linear solver steps done is " << 
  std::cout << " The total number of iterative linear solver steps done is " << 
               info.noLiniterIterations << std::endl;
  std::string finalOutputFilename;
  if ( info.returnCode==0 ) 
    finalOutputFilename="graph/sst_nleqErr_solution";
  else 
    finalOutputFilename="graph/sst_nleqErr_final";
