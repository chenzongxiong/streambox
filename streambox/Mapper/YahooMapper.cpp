...
template <>
uint64_t YahooMapper<Event1, pair<creek::string, long>, RecordBundle>::do_map(Record<Event1> const & in, shared_ptr<RecordBundle<pair<creek::string,long>>> output_bundle)
{
...
template<>
uint64_t YahooMapper<Event1, creek::string, RecordBundle>::do_map(Record<Event1> const & in, shared_ptr<RecordBundle<creek::string>> output_bundle)
{
    return 0;
}
...
template
void YahooMapper<Event1, creek::string,
        RecordBundle>::ExecEvaluator
        (int nodeid, EvaluationBundleContext *c,
         shared_ptr<BundleBase> bundle = nullptr);

template
void YahooMapper<Event1, pair<creek::string, long>,
        RecordBundle>::ExecEvaluator
        (int nodeid, EvaluationBundleContext *c,
         shared_ptr<BundleBase> bundle = nullptr);
